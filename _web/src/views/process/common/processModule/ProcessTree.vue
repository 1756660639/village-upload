...
import End from './nodes/EndNode.vue'

import DefaultProps from './DefaultNodeProps'

export default {
  name: 'ProcessTree',
  components: { Node, Root, Approval, Cc, Concurrent, Condition, Delay, Empty, End },
  data() {
    return {
      valid: true
    }
  ...
  methods: {
    ...
    isPrimaryNode(node) {
      return node &&
          ...
          node.type === 'CC' || node.type === 'DELAY' || node.type === 'END')
    },
    isBranchNode(node) {
    ...
    // 选中一个节点
    selectNode(node) {
      this.$store.commit('process/selectedNode', node)
