<template>
  <div v-if="show" ref="duoViewer" class="duo-viewer" @click="clickAgent">
    ...
    <div class="duo-viewer-footer">
      <div class="duo-viewer-footer__title">{{ imgName }}</div>
      <div class="duo-viewer-footer__toolbar">
        <ul>
          ...
          />
          <li class="duo-viewer-footer__prev" data-viewer-action="prev" />

          <li class="duo-viewer-footer__play" data-viewer-action="play" />

          <li class="duo-viewer-footer__next" data-viewer-action="next" />
          <li class="duo-viewer-footer__reset" data-viewer-action="reset" />
          <li
    ...
    <div class="duo-viewer-close" @click="handleClose">
      <div class="duo-viewer-close__off" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    resetPosition() {
      ...
      const oldTransform = this.getCurrentTransform(image)
    ...
    rotate(type) {
      const image = this.image
      const oldTransform = this.getCurrentTransform(image)
      const oldRotate = Math.round(
        Math.atan2(+oldTransform.skewY, +oldTransform.scaleX) *
            (180 / Math.PI)
      )
      const changeValue = type === 'left' ? -90 : 90
      const rotateValue = oldRotate + changeValue
    ...
    switchAction(a) {
      ...
      switch (a) {
        ...
        case 'next':
          // next action
          const srcListLength = this.listLength - 1
    ...
    judgeThumbnailListMove(lastIndex, index) {
      ...
      const target = this.$refs['duoViewerImageThumbnailList']
    ...
    zoom(type) {
      const image = this.image
      const w = +this.getStyleByName(image, 'width').split('px')[0]
      const h = +this.getStyleByName(image, 'height').split('px')[0]
    ...
    // FullScreen action
    requestFullScreen() {
      const element = this.$refs['duoViewer']
      const requestMethod =
